...
await m.reply(`Ayrton-Bot: ocurrió un error.`)
console.log(e)
}
...
await conn.sendMessage(m.chat, { text: `Ayrton-Bot: no hay sesión activa.\nUsa: ${usedPrefix + command}\nSi tienes ID:\n${usedPrefix + command} (ID)` }, { quoted: m })
return
}
if (global.conn.user.jid !== conn.user.jid) {
  ...
  await conn.sendMessage(m.chat, { text: `Ayrton-Bot: sub-bot desconectado.` }, { quoted: m })
}
try {
...
if (global.conn.user.jid == conn.user.jid) {
  ...
  await conn.reply(m.chat, `Ayrton-Bot desactivado.`, m)
  conn.ws.close()
}  
break
